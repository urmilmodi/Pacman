...
module Clyde
    ...
    always@(posedge sysclk) begin
        case(current_state)
            ...
            randomDirections: next_state <= update ? DistoPac : clearWalls;
            DistoPac: next_state <= update ? targetSelection : clearWalls;
    ...
    // Clyde reverts to Scatter if Distance to Pacman is < 8*Pacmansize
    wire [16:0] disToPac;
    
    dis Pac(currentloc, pacloc, 1, sysclk, disToPac);

    // Compares to 64*size*size because dis module returns the distance squared
    wire [3:0] InternalMode = (disToPac <= 64*8'd14*8'd14) ? Scatter : mode;
