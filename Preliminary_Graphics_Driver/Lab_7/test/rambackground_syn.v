...
//altsyncram CLOCK_ENABLE_INPUT_A="BYPASS" CLOCK_ENABLE_OUTPUT_A="BYPASS" DEVICE_FAMILY="Cyclone V" ENABLE_RUNTIME_MOD="NO" INIT_FILE="white.mif" NUMWORDS_A=19200 OPERATION_MODE="SINGLE_PORT" OUTDATA_ACLR_A="NONE" OUTDATA_REG_A="UNREGISTERED" POWER_UP_UNINITIALIZED="FALSE" read_during_write_mode_port_a="NEW_DATA_NO_NBE_READ" WIDTH_A=3 WIDTH_BYTEENA_A=1 WIDTHAD_A=15 address_a clock0 data_a q_a wren_a
...
`endif
	...
	assign
		...
		eq = eq_node,
		eq_node = eq_wire[2:0],
		eq_wire = {w_anode100w[2], w_anode92w[2], w_anode84w[2], w_anode71w[2]},
		w_anode100w = {(w_anode100w[1] & data_wire[1]), (w_anode100w[0] & data_wire[0]), enable_wire},
		w_anode71w = {(w_anode71w[1] & (~ data_wire[1])), (w_anode71w[0] & (~ data_wire[0])), enable_wire},
		w_anode84w = {(w_anode84w[1] & (~ data_wire[1])), (w_anode84w[0] & data_wire[0]), enable_wire},
		w_anode92w = {(w_anode92w[1] & data_wire[1]), (w_anode92w[0] & (~ data_wire[0])), enable_wire};
endmodule //rambackground_decode
...
`endif
	...
	wire  [2:0]  eq_node;
	wire  [3:0]  eq_wire;
	wire  [2:0]  w_anode109w;
	wire  [2:0]  w_anode123w;
	wire  [2:0]  w_anode132w;
	wire  [2:0]  w_anode141w;

	assign
		data_wire = data,
		eq = eq_node,
		eq_node = eq_wire[2:0],
		eq_wire = {w_anode141w[2], w_anode132w[2], w_anode123w[2], w_anode109w[2]},
		w_anode109w = {(w_anode109w[1] & (~ data_wire[1])), (w_anode109w[0] & (~ data_wire[0])), 1'b1},
		w_anode123w = {(w_anode123w[1] & (~ data_wire[1])), (w_anode123w[0] & data_wire[0]), 1'b1},
		w_anode132w = {(w_anode132w[1] & data_wire[1]), (w_anode132w[0] & (~ data_wire[0])), 1'b1},
		w_anode141w = {(w_anode141w[1] & data_wire[1]), (w_anode141w[0] & data_wire[0]), 1'b1};
endmodule //rambackground_decode1
...
endmodule //rambackground_mux

//synthesis_resources = lut 14 M10K 2 M10K 6 reg 2 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
(* ALTERA_ATTRIBUTE = {"OPTIMIZE_POWER_DURING_SYNTHESIS=NORMAL_COMPILATION"} *)
module  rambackground_altsyncram
	( 
	address_a,
	clock0,
	data_a,
...
`endif
	...
	rambackground_decode   decode3
	( 
	.data(address_a_wire[14:13]),
	.enable(wren_a),
	.eq(wire_decode3_eq));
	rambackground_decode1   rden_decode
	( 
	.data(rden_decode_addr_sel_a),
	.eq(wire_rden_decode_eq));
	rambackground_mux   mux2
	( 
...
endmodule //rambackground_altsyncram
//VALID FILE
...
`endif
	...
	rambackground_altsyncram	rambackground_altsyncram_component (
				.address_a (address),
				.clock0 (clock),
				.data_a (data),
				.wren_a (wren),
